"""在本地数据集上训练 Diffusion Policy 策略的示例脚本（适配 SO-101 + Windows + CUDA）。

Diffusion Policy 是基于扩散模型的策略学习方法：
- 架构：ResNet18 视觉编码器 + 1D U-Net 动作生成模型
- 原理：从随机噪声出发，逐步"去噪"生成一段平滑的动作序列（默认32步）
- 优势：天然生成平滑轨迹，对机器人控制更友好
- 显存需求：~6-8GB（batch_size=32），介于 ACT 和 PI0 之间
- 训练速度：中等（比 ACT 慢，比 PI0 快）

使用方式（在已激活 lerobot 环境的终端中运行）：
    python examples/tutorial/diffusion/diffusion_training_example_so101.py

训练完成后，模型 checkpoint 会保存到 outputs/robot_learning_tutorial/diffusion_so101/ 目录。
"""
...
def main():
    # === 1. 输出目录 ===
    output_directory = Path("outputs/robot_learning_tutorial/diffusion_so101")
