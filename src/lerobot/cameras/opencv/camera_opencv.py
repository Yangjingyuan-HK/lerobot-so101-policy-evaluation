...
class OpenCVCamera(Camera):
    ...
    def _validate_fps(self) -> None:
        ...
        # DSHOW 等后端可能报告 FPS=-1 或 0，这不是真正的失败
        if not success or (actual_fps > 0 and not math.isclose(self.fps, actual_fps, rel_tol=1e-3)):
            logger.warning(
                f"{self} failed to set fps={self.fps} ({actual_fps=}, {success=}). "
                f"Continuing with default FPS."
            )
    ...
    def _read_loop(self) -> None:
        ...
        while not stop_event.is_set():
            try:
                ...
                failure_count += 1
                if failure_count <= 50:
                    logger.warning(f"Error reading frame in background thread for {self} (attempt {failure_count}/50): {e}")
                    time.sleep(0.1)
                    # 每 10 次失败尝试重新连接摄像头
                    if failure_count % 10 == 0 and self.videocapture is not None:
                        logger.warning(f"Attempting to reconnect {self}...")
                        try:
                            self.videocapture.release()
                            time.sleep(0.5)
                            self.videocapture = cv2.VideoCapture(self.index_or_path, self.backend)
                            if self.videocapture.isOpened():
                                self._configure_capture_settings()
                                logger.warning(f"Reconnected {self} successfully.")
                            else:
                                logger.warning(f"Reconnect failed for {self}.")
                        except Exception as reconnect_err:
                            logger.warning(f"Reconnect error for {self}: {reconnect_err}")
                else:
                    raise RuntimeError(f"{self} exceeded maximum consecutive read failures (50).") from e
