...
class TrainPipelineConfig(HubMixin):
    ...
    def _resolve_pretrained_from_cli(self) -> None:
        ...
        if reward_model_path:
            ...
            self.policy.pretrained_path = str(Path(policy_path)).replace("\\", "/")
        elif self.resume:
    ...
    def validate(self) -> None:
        ...
        # Normalize pretrained_path to forward-slash strings for cross-platform Hub compatibility.
        # On Windows, Path objects stringify with backslashes (e.g. 'lerobot\pi0_base'), which are
        # invalid in HuggingFace Hub repo IDs and cause HFValidationError. This covers all entry
        # points (--policy.path=, --policy.pretrained_path=, config files) so the path is always
        # a forward-slash string before any downstream hf_hub_download / cached_file call.
        for cfg in (self.policy, self.reward_model):
            if cfg is not None and cfg.pretrained_path is not None:
                cfg.pretrained_path = str(cfg.pretrained_path).replace("\\", "/")
